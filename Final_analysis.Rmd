...
Reading the file
```{r}
main_data <- read.table("GSE201316_allCounts.txt", header = TRUE, sep = "\t", row.names = 1)
...
treatment <- c("CD", "CD", "HFD", "HFD", "HFD", "HFD", "CD", "CD", "HFD", "HFD", "CD", "CD", "HFD", "HFD", "HFD", "HFD", "CD", "CD", "HFD", "HFD","CD", "CD")
tumor_status <- c("T", "C", "T", "C", "T", "C", "T", "C", "T", "C", "T", "C", "T", "C", "T", "C", "T", "C", "T", "C", "T", "C")
...
meta_data <- data.frame(samples, treatment, tumor_status, group, stringsAsFactors = FALSE, row.names = 1)
```
...
```{r warning=FALSE}
dds <- DESeqDataSetFromMatrix(countData = main_data, colData = meta_data, design = ~tumor_status)
dds
```
pre-filtering 
```{r}
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
library(pheatmap)
pheatmap(vsd_cor, annotation = dplyr::select(meta_data, c("treatment", "tumor_status")))
```

visualizing PCA plot
```{r}
p <- plotPCA(vsd, intgroup = c("treatment", "tumor_status"))
...
```{r}
l2f <- results(dds, name ="treatmentHFD.tumor_statusT" , alpha = 0.05)
```
...
pheatmap(sig_norm_counts, color = heat_colors, cluster_rows = TRUE, show_rownames = FALSE, annotation = dplyr::select(meta_data,treatment), scale = "row")
```
...
```{r}
expression <- sig_norm_counts[7,]
group <- meta_data[,"group"]
boxplot(expression ~group, main = l2f_sig[7,"ensgene"])
